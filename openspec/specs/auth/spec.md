...
本 capability 定義 vehicle management 系統的「認證 (authentication) 與 session」機制：包含登入、登出、取得當前使用者、密碼儲存策略、JWT/HttpOnly cookie session、CSRF 防護、brute-force 鎖定、以及保護受限 route 的認證 middleware。本系統 SHALL NOT 提供 self-service 自助註冊；所有帳號皆由 admin 透過 employees capability 建立。
