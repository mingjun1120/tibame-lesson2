...
本 capability 定義系統首頁 dashboard 的「角色感知 (role-aware) 彙整資料」：一個依角色限縮 scope 的 summary endpoint、6 張 summary card、3 張 chart，以及對應的前端視圖。admin 取得全公司統計；user 僅看到 `ownerId` 等於自身 `employeeId` 的 vehicle 統計。對於因角色限縮而從 API 缺席的 card 或 chart，前端 SHALL 完全不渲染。
